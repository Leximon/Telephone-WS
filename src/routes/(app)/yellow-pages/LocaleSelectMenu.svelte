...
<div class="relative" use:outsideClick={() => open = false}>
    <button class="transparent px-2 h-full w-32 max-sm:w-24 duration-150 hover:bg-slate-300" on:click={toggle}>
    ...
    {#if open}
        <div class="absolute top-full left-0 right-0 bg-slate-200 shadow-lg rounded-b-lg max-h-32 overflow-y-scroll overflow-x-hidden z-50 [&_p]:text-left" transition:slide={{duration: 150}}>
            <button class="transparent w-full h-full duration-150 p-1 hover:bg-slate-300 border-b border-gray-400 last:border-b-0" on:click={() => select(null)}>
                <p class="text-sm max-sm:text-xs m-0 text-gray-500 italic">Deselect</p>
            </button>
            {#each Object.entries(DiscordLocaleValues) as [key, value]}
                <button class="transparent w-full h-full duration-150 p-1 hover:bg-slate-300 border-b border-gray-400 last:border-b-0 flex flex-row" on:click={() => select(key)}>
                    <div class="flex items-center mr-1">
