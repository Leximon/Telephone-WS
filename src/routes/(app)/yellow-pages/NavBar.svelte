...
<div class="flex justify-between" class:mt-2={bottom} class:mb-2={!bottom}>
    <div>
        <button on:click={() => dispatch("flip", -page)} class="group rounded-full hover:bg-opacity-20 hover:bg-black duration-150 disabled:bg-transparent disabled:cursor-default" disabled={page <= 0}>
        ...
        <button on:click={() => dispatch("flip", -1)} class="group rounded-full hover:bg-opacity-20 hover:bg-black duration-150 disabled:bg-transparent disabled:cursor-default" disabled={page <= 0}>
            <i class="fa-solid fa-angle-left text-stone-800 mx-2 group-disabled:text-stone-400"></i>
        </button>
    </div>
